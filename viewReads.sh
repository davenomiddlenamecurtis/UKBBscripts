...
if [ ! -e $targetDir/$name.countsByStrand.txt ]
then
  mkdir $targetDir/$name
  realChr=$chr
  if [ $chr == 23 ]
  ...
  $bamReadcount -i -f $refDir/chr$chr.fa $targetDir/$name/$name.bam chr$chr:$pos-$pos > $targetDir/$name/$name.counts.txt
  chr=$realChr
  read -a words < $targetDir/$name/$name.counts.txt
  counts=""
  for w in {4..10}
  do
    if [ -z ${words[$w]} ] # only 10 for indels
	then
	  continue
	fi
